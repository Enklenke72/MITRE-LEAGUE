...
- **Hoy es solo cosmético:** `<select id="selector-rol-usuario">` alterna `rol-admin`/`rol-staff` en `<body>` y el CSS oculta `.solo-admin`. No hay seguridad real. **Este selector desaparece por completo cuando se active Firebase Auth** (Hecho #18, Pendiente y 11.4 punto 12): el rol deja de elegirse a mano y pasa a leerse del documento del usuario logueado en Firestore (colección `usuarios`), en un login real por email/contraseña. No es "ocultarlo mejor", es sacarlo.
- El login de `#pantalla-admin` en `index.html` **ya tiene el enganche de JS armado** (`JAVASCRIPT/auth-login.js`, formulario con validación y mensaje de error), pero **todavía no autentica de verdad**: no existe el proyecto de Firebase. Hoy el botón siempre responde "acceso no activado". Ver Hecho #18 y `Claude outputs/preparacion-login-firebase.md`.
...
11. **Login de staff: enganche ya preparado (sin autenticar todavía).** Otra sesión dejó `JAVASCRIPT/auth-login.js` (enganchado desde `index.html`, valida que los campos no estén vacíos y muestra "acceso todavía no activado") y `JAVASCRIPT/firebase-config.js` (plantilla vacía a propósito, sin claves inventadas), más `Claude outputs/preparacion-login-firebase.md` con los pasos. Cuando exista el proyecto de Firebase, activar el login real es reemplazar `iniciarSesionStaff` y completar la config: no hay que rehacer HTML ni CSS.
